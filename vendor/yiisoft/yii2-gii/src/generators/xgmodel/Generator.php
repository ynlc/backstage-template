...
namespace yii\gii\generators\xgmodel;
...
class Generator extends \yii\gii\Generator
{
    ...
    public function generate()
    {
        ...
        foreach ($this->getTableNames() as $tableName) {
            ...
            $newPrefix = ucfirst(trim(str_replace('_', '', $prefix)));
            $modelClassName = $newPrefix.$this->generateClassName($tableName);
            $cmodelClassName = str_replace($newPrefix, 'C', $modelClassName);
            ...
            $cparams = [
                'xgclassName' => $modelClassName,
                'className' => $cmodelClassName
            ];
            $files[] = new CodeFile(
                Yii::getAlias('@common/models/giiCreated') . '/' . $modelClassName . '.php',
                $this->render('model.php', $params)
            );
            $files[] = new CodeFile(
                Yii::getAlias('@common/models') . '/' . $cmodelClassName . '.php',
                $this->render('cmodel.php', $cparams)
            );
            // query :
            if ($queryClassName) {
